...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for whived is necessary for this upgrade with the "reindex=1" whive.conf option. \n' +
                 'There are changes necessary in both whive.conf and whivecore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/whiveevo/whivecore-node/blob/whived/docs/upgrade.md \n');
        ...
        servicesConfig: {
          whived: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/whived')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed whivecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './whivecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in whivecore-node services directory
    ...
    if (servicePackage.whivecoreNode) {
      serviceModule = service.name + '/' + servicePackage.whivecoreNode;
    }
