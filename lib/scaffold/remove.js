...
/**
 * Will remove a service from whivecore-node.json
...
/**
 * Will remove the Node.js service and from the whivecore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The whivecore-node configuration directory
...
function remove(options, done) {
  ...
  var whivecoreConfigPath = path.resolve(configPath, 'whivecore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(whivecoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a whivecore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to whivecore-node.json
        removeConfig(whivecoreConfigPath, service, next);
      });
    }, done
  );
}
