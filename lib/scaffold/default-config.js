...
/**
 * Will return the path and default whivecore-node configuration. It will search for the
 * configuration file in the "~/.whivecore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.whivecore');
  var defaultConfigFile = path.resolve(defaultPath, './whivecore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['whived', 'web'];
      ...
      servicesConfig: {
        whived: {
          connect: [{
            ...
            rpcuser: 'whive',
            rpcpassword: 'pass',
            zmqpubrawtx: 'tcp://127.0.0.1:21867'
          }]
        }
      }
    };
