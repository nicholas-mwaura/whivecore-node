...
var DashdRPC = require('@whiveevo/whived-rpc');
var $ = whivecore.util.preconditions;
var _  = whivecore.deps._;
var Transaction = whivecore.Transaction;
var Proposal = whivecore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to whived in Node.js. Manages starting and
 * stopping whived as a child process for application support, as well as connecting
 * to multiple whived processes for server infrastructure. Results are cached in an
...
Dash.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'whived/' + name, 'total:', this.subscriptions[name].length);
};
...
Dash.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'whived/' + name, 'total:', this.subscriptions[name].length);
};
...
Dash.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (whivecore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'whived/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Dash.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'whived/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in whived config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in whived config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in whived config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './whive.conf');
...
Dash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of whivecore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of whivecore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of whivecore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in whive.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that whived is undergoing a reindex. ' +
             ...
             'of whivecore-node services will start.');
    node._reindex = true;
  }
};
...
Dash.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === whivecore.Networks.testnet) {
...
Dash.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === whivecore.Networks.testnet) {
    networkPath = 'testnet3/whive.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/whive.conf';
    }
  }
  return networkPath;
};
...
Dash.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('whived/hashblock', message.toString('hex'));
    }
  }
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('whived/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Dash.prototype._zmqTransactionHandler = function(node, message) {
  ...
  var self = this;
  var hash = whivecore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('whived/rawtransaction', message.toString('hex'));
    }

    var tx = whivecore.Transaction();
    tx.fromString(message);
    var txid = whivecore.util.buffer.reverse(hash).toString('hex');
...
Dash.prototype._zmqTransactionLockHandler = function (node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('whived/transactionlock', message.toString('hex'));
    }
...
Dash.prototype._stopSpawnedDash = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/whived.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned whive process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean whive process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedDash(function(err) {
    ...
    log.info('Starting whive process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        ...
        log.warn('Restarting whive child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn whived.'));
      }
...
Dash.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to whived.'));
    }
 ...
 * @param {String|Address|Array} addressArg - An address string, whivecore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Dash.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = whivecore.Script.fromAddress(delta.address);
    return {
...
Dash.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in whivecore is most recent to less recent
 ...
 * @param {Object} txid - A whive transaction id
 * @param {Function} callback
 */
...
Dash.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof whivecore.Address) {
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = whivecore.Block.fromString(response.result);
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('whived spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('whived process did not exit'));
      }
